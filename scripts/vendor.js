...
(function(name, context, definition) {
	...
	var win = window, old = context[name], namespaceRegex = /[^\.]*(?=\..*)\.|.*/, nameRegex = /\..*/, addEvent = 'addEventListener', removeEvent = 'removeEventListener', doc = document || {}, root = doc.documentElement || {}, W3C_MODEL = root[addEvent], eventSupport = W3C_MODEL ? addEvent : 'attachEvent', ONE = {}// singleton for quick matching making add() do one()
	...
	, Event = ( function() {
			...
			var commonProps = str2arr('altKey attrChange attrName bubbles cancelable ctrlKey currentTarget ' + 'detail eventPhase getModifierState isTrusted metaKey relatedNode relatedTarget shiftKey ' + 'srcElement target timeStamp type view which propertyName'), mouseProps = commonProps.concat(str2arr('button buttons clientX clientY dataTransfer ' + 'fromElement offsetX offsetY pageX pageY screenX screenY toElement')), mouseWheelProps = mouseProps.concat(str2arr('wheelDelta wheelDeltaX wheelDeltaY wheelDeltaZ ' + 'axis'))// 'axis' is FF specific
			, keyProps = commonProps.concat(str2arr('char charCode key keyCode keyIdentifier ' + 'keyLocation location')), textProps = commonProps.concat(str2arr('data')), touchProps = commonProps.concat(str2arr('touches targetTouches changedTouches scale rotation')), messageProps = commonProps.concat(str2arr('data origin source')), stateProps = commonProps.concat(str2arr('state')), overOutRegex = /over|out/
			...
			, Event = function(event, element, isNative) {
				if (!arguments.length)
					return event = event || ((element.ownerDocument || element.document || element).parentWindow || win).event
	...
	/**
	 * Bean maintains an internal registry for event listeners. We don't touch elements,
	  objects
	...
	, listener = W3C_MODEL ? function(element, type, add) {
		...
		var entry
		if (add) {
			registry.put( entry = new RegEntry(element, custom || type, function(event) {// handler
	...
	/**
	 * on(element,
	  eventType(s)[, selector], handler[, args ])
	 */
	...
	/**
	 * add(element[,
	  selector], eventType(s), handler[, args ])
	 *
...
(function() {
	...
	b2Collision.CollidePolygons = function(manifold, polyA, xfA, polyB, xfB) {
		...
		for (var i = 0; i < b2Settings.b2_maxManifoldPoints; ++i) {
			...
			if (separation <= totalRadius) {
				...
				cp.m_id.Set(cv.id);
				cp.m_id.features.flip = flip; ++pointCount;
			}
		}
	...
	b2Distance.Distance = function(output, cache, input) {++b2Distance.b2_gjkCalls;
		...
		while (iter < k_maxIters) {
			...
			vertex.w = b2Math.SubtractVV(vertex.wB, vertex.wA); ++iter; ++b2Distance.b2_gjkIters;
			var duplicate = false;
			for ( i = 0; i < saveCount; i++) {
	...
	b2DynamicTreeBroadPhase.prototype.CreateProxy = function(aabb, userData) {
		var proxy = this.m_tree.CreateProxy(aabb, userData); ++this.m_proxyCount;
		this.BufferMove(proxy);
		return proxy;
	}
	b2DynamicTreeBroadPhase.prototype.DestroyProxy = function(proxy) {
		this.UnBufferMove(proxy); --this.m_proxyCount;
	...
	b2DynamicTreeBroadPhase.prototype.UpdatePairs = function(callback) {
		...
		for (var i = 0; i < __this.m_pairCount; ) {
			...
			callback(userDataA, userDataB); ++i;
	...
	b2TimeOfImpact.TimeOfImpact = function(input) {++b2TimeOfImpact.b2_toiCalls;
		...
		for (; ; ) {
			...
			var newAlpha = alpha;
			{
				...
				for (; ; ) {
					...
					if (f > target) {
						...
						x2 = x;
						f2 = f;
					}++rootIterCount; ++b2TimeOfImpact.b2_toiRootIters;
					if (rootIterCount == 50) {
						break;
					}
				}
			...
			alpha = newAlpha;
			iter++; ++b2TimeOfImpact.b2_toiIters;
...
(function() {
	...
	b2Body.prototype.CreateFixture = function(def) {
		...
		this.m_fixtureList = fixture; ++this.m_fixtureCount;
		fixture.m_body = this;
	...
	b2Body.prototype.DestroyFixture = function(fixture) {
		...
		fixture.Destroy();
		fixture.m_body = null;
		fixture.m_next = null; --this.m_fixtureCount;
		this.ResetMassData();
	}
	...
	b2ContactManager.prototype.AddPair = function(proxyUserDataA, proxyUserDataB) {
		...
		bodyB.m_contactList = c.m_nodeB; ++this.m_world.m_contactCount;
		return;
	}
	...
	b2World.prototype.CreateBody = function(def) {
		...
		this.m_bodyList = b; ++this.m_bodyCount;
		return b;
	}
	...
	b2World.prototype.CreateJoint = function(def) {
		...
		this.m_jointList = j; ++this.m_jointCount;
		j.m_edgeA.joint = j;
	...
	b2World.prototype.DestroyJoint = function(j) {
		...
		j.m_edgeB.prev = null;
		j.m_edgeB.next = null;
		b2Joint.Destroy(j, null); --this.m_jointCount;
	...
	b2World.prototype.CreateController = function(controller) {
		...
		this.m_controllerList = controller; ++this.m_controllerCount;
		...
		for (; ; ) {
			...
			while (queueSize > 0) {
				...
				for ( cEdge = b.m_contactList; cEdge; cEdge = cEdge.next) {
					...
					queue[queueStart + queueSize] = other; ++queueSize;
					other.m_flags |= b2Body.e_islandFlag;
				}
